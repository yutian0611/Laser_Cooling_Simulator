...
while (t < 300):
    ...
    for i in range(0, intensity):
        temp_rand = int(np.random.normal(loc=25.0, scale=1.0, size=None))
        if (random.uniform(0, 1) > 0.5 and temp_rand > 20 and temp_rand < 30):
            temp_photon = Photon(temp_rand, np.pi, width+random.uniform(0,10), random.uniform(height / 2 - 100, height / 2 + 100))
            all_incident_photons.append(temp_photon)
            temp_dot = w.create_oval(temp_photon.pos[0] - 5, temp_photon.pos[1] - 5, temp_photon.pos[0] + 5, temp_photon.pos[1] + 5, fill="#" + str(temp_photon.wavelength - 20) + "0" + str(30 - temp_photon.wavelength))
    ...
    for i in range(0, number_of_atoms):
        ...
        for k in all_incident_photons:
            ...
            if (distance <= radius and (all_atoms[i].v[0] - k.v[0]) / k.wavelength > 0.95 and (all_atoms[i].v[0] - k.v[0]) / k.wavelength < 1.05):
                ...
                temp_dot = w.create_oval(temp_emitted.pos[0] - 5, temp_emitted.pos[1] - 5, temp_emitted.pos[0] + 5, temp_emitted.pos[1] + 5, fill='black')
